...
<head>
	...
	<title>MongoDB Stitch Server SDK | MongoDB Stitch Server SDK</title>
...
<div class="container container-main">
	<div class="content-wrap">
		<div class="tsd-panel tsd-typography">
			...
			<h1 id="mongodb-stitch-server-sdk">MongoDB Stitch Server SDK</h1>
			<p>The official <a href="https://stitch.mongodb.com/">MongoDB Stitch</a> Server SDK for JavaScript/TypeScript.</p>
			...
			<ul>
				<li><a href="https://s3.amazonaws.com/stitch-sdks/js-server/docs/4.0.14/index.html">API/Typedoc Documentation</a></li>
			...
			<pre><code class="lang-bash">npm install mongodb-stitch-server-sdk
</code></pre>
			...
			<pre><code class="lang-bash">npm install mongodb-stitch-server-core
...
npm install mongodb-stitch-server-services-twilio
			...
			<ol>
				...
				<li>Add the MongoDB Stitch Server SDK by running <code>npm install mongodb-stitch-server-sdk</code>.</li>
				<li>Create directories for your source files:</li>
			</ol>
    ...
    Stitch, 
    AnonymousCredential,
} = <span class="hljs-built_in">require</span>(<span class="hljs-string">'mongodb-stitch-server-sdk'</span>);
...
client.auth.loginWithCredential(<span class="hljs-keyword">new</span> AnonymousCredential()).then(<span class="hljs-function"><span class="hljs-params">user</span> =&gt;</span> {
    ...
    client.close();
			...
			<ol start="6">
				<li>Run the app by running <code>node src/index.js</code>.</li>
			</ol>
			...
			<pre><code class="lang-javascript"><span class="hljs-keyword">const</span> { Stitch, AnonymousCredential } = <span class="hljs-built_in">require</span>(<span class="hljs-string">'mongodb-stitch-server-sdk'</span>);
...
logged <span class="hljs-keyword">in</span> anonymously as<span class="hljs-built_in"> user </span>58c5d6ebb9ede022a3d75050
  ...
  <span class="hljs-built_in">console</span>.log(<span class="hljs-string">`Echoed result: <span class="hljs-subst">${echoedResult}</span>`</span>);
			...
			<h4 id="changing-the-default-data-directory">Changing the default data directory</h4>
			<p>Clients will generally store their data in <code>$HOME/.stitch/&lt;your-client-app-id&gt;</code>. To modify this directory, you must initialize an app client with a custom data directory set:</p>
			<pre><code class="lang-javascript"><span class="hljs-keyword">const</span> { Stitch, StitchAppClientConfiguration } = <span class="hljs-built_in">require</span>(<span class="hljs-string">'mongodb-stitch-server-sdk'</span>);

<span class="hljs-keyword">const</span> client = Stitch.initializeAppClient(<span class="hljs-string">"&lt;your-client-app-id&gt;"</span>, <span class="hljs-keyword">new</span> StitchAppClientConfiguration.Builder().withDataDirectory(<span class="hljs-string">'/some/path'</span>).build());
</code></pre>
			<p>You can use the client returned there or anywhere else in your app by using the following:</p>
			<pre><code class="lang-javascript"><span class="hljs-keyword">const</span> client = Stitch.getAppClient(<span class="hljs-string">"&lt;your-client-app-id&gt;"</span>);
</code></pre>
			<h4 id="closing-the-stitchappclient">Closing the StitchAppClient</h4>
			<p>The client maintains some background processes in the event loop that must be shutdown when the client is no longer needed. Simply call <code>close</code> on the client when you are done with the client:</p>
