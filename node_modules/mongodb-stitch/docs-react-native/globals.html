...
<head>
	...
	<title>MongoDB Stitch React Native SDK | MongoDB Stitch React Native SDK</title>
...
<div class="container container-main">
	<div class="content-wrap">
		<div class="tsd-panel tsd-typography">
			...
			<h1 id="mongodb-stitch-react-native-sdk">MongoDB Stitch React Native SDK</h1>
			<p>The official <a href="https://stitch.mongodb.com/">MongoDB Stitch</a> React Native SDK for JavaScript/TypeScript.</p>
			...
			<ul>
				<li><a href="https://s3.amazonaws.com/stitch-sdks/js-react-native/docs/4.0.14/index.html">API/Typedoc Documentation</a></li>
			...
			<pre><code class="lang-bash">npm install mongodb-stitch-react-native-sdk
</code></pre>
			...
			<pre><code class="lang-bash">npm install mongodb-stitch-react-native-core
...
npm install mongodb-stitch-react-native-services-twilio
			...
			<h3 id="creating-a-new-app-with-the-sdk-react-native-">Creating a new app with the SDK (React Native)</h3>
			...
			<h4 id="set-up-a-react-native-project">Set up a React Native project</h4>
			<ol>
				<li>Ensure that you have <code>npm</code> installed. See <a href="https://www.npmjs.com">npmjs.com</a>.</li>
				<li>If you don&#39;t already have <a href="https://github.com/react-community/create-react-native-app">Create React Native App</a> installed, run <code>npm install -g create-react-native-app</code></li>
				<li>Initialize a new React Native project with <code>create-react-native-app YourProjectName</code>.</li>
				<li>Go to the new project with <code>cd YourProjectName</code>.</li>
				<li><p>Add the MongoDB Stitch React Native SDK by running <code>npm install mongodb-stitch-react-native-sdk</code>.</p>
				</li>
				<li><p>In <code>App.js</code>, replace the existing code with the following, replacing <code>&lt;your-client-app-id&gt;</code> with the id you retrieved when setting up the application in MongoDB Stitch:</p>
				</li>
			</ol>
			<pre><code class="lang-jsx"><span class="hljs-keyword">const</span> React = <span class="hljs-built_in">require</span>(<span class="hljs-string">'react'</span>);
<span class="hljs-keyword">const</span> { Button, StyleSheet, Text, View } = <span class="hljs-built_in">require</span>(<span class="hljs-string">'react-native'</span>);
<span class="hljs-keyword">const</span> { Stitch, AnonymousCredential } = <span class="hljs-built_in">require</span>(<span class="hljs-string">'mongodb-stitch-react-native-sdk'</span>);
...
<span class="hljs-keyword">export</span> <span class="hljs-keyword">default</span> <span class="hljs-class"><span class="hljs-keyword">class</span> <span class="hljs-title">App</span> <span class="hljs-keyword">extends</span> <span class="hljs-title">React</span>.<span class="hljs-title">Component</span> </span>{
  <span class="hljs-keyword">constructor</span>(props) {
    ...
    <span class="hljs-keyword">this</span>.state={
      <span class="hljs-attr">currentUserId</span>: <span class="hljs-literal">undefined</span>,
      <span class="hljs-attr">client</span>: <span class="hljs-literal">undefined</span>
    };
    <span class="hljs-keyword">this</span>._loadClient = <span class="hljs-keyword">this</span>._loadClient.bind(<span class="hljs-keyword">this</span>);
    <span class="hljs-keyword">this</span>._onPressLogin = <span class="hljs-keyword">this</span>._onPressLogin.bind(<span class="hljs-keyword">this</span>);
    <span class="hljs-keyword">this</span>._onPressLogout = <span class="hljs-keyword">this</span>._onPressLogout.bind(<span class="hljs-keyword">this</span>);
  }

  componentDidMount() {
    <span class="hljs-keyword">this</span>._loadClient();
  }

  render() {
    <span class="hljs-keyword">let</span> loginStatus = <span class="hljs-string">"Currently logged out."</span>

    <span class="hljs-keyword">if</span>(<span class="hljs-keyword">this</span>.state.currentUserId) {
      loginStatus = <span class="hljs-string">`Currently logged in as <span class="hljs-subst">${<span class="hljs-keyword">this</span>.state.currentUserId}</span>!`</span>
    }
...
const styles = StyleSheet.create({
  container: {
    ...
    justifyContent: 'center',
			...
			<ol start="7">
				<li>Run the app by running <code>npm start</code> and following the instructions in the terminal. You may need to run <code>npm install</code> before <code>npm start</code> works correctly.</li>
			</ol>
			<h3 id="using-the-sdk">Using the SDK</h3>
			<h4 id="initialize-the-sdk">Initialize the SDK</h4>
			<ol>
				<li>When your app has started, run the following code to initialize the Stitch SDK, replacing <code>&lt;your-client-app-id&gt;</code> with your Stitch application&#39;s client app ID:</li>
			</ol>
			<pre><code class="lang-javascript"><span class="hljs-keyword">const</span> { Stitch, AnonymousCredential } = <span class="hljs-built_in">require</span>(<span class="hljs-string">'mongodb-stitch-react-native-sdk'</span>);

Stitch.initializeDefaultAppClient(<span class="hljs-string">'&lt;your-client-app-id&gt;'</span>).then(<span class="hljs-function"><span class="hljs-params">client</span> =&gt;</span> {
    <span class="hljs-comment">// use the client</span>
});
</code></pre>
			<p>Please take note that unlike the other Stitch SDKs, this client initialization
			...
			system.</p>
			<ol start="2">
				<li>To get an already-initialized client to use for logging in and communicating
				with Stitch, use <code>Stitch.defaultAppClient</code></li>
			</ol>
...
client.auth.loginWithCredential(<span class="hljs-keyword">new</span> AnonymousCredential()).then(<span class="hljs-function"><span class="hljs-params">user</span> =&gt;</span> {
  <span class="hljs-built_in">console</span>.log(<span class="hljs-string">`logged in anonymously as user <span class="hljs-subst">${user.id}</span>`</span>)
			...
			<ol start="2">
				<li>When running this code, you should see the following in your standard out:</li>
			</ol>
...
logged <span class="hljs-keyword">in</span> anonymously as<span class="hljs-built_in"> user </span>58c5d6ebb9ede022a3d75050
</code></pre><h4 id="executing-a-function">Executing a function</h4>
			<ol>
  ...
  <span class="hljs-built_in">console</span>.log(<span class="hljs-string">`Echoed result: <span class="hljs-subst">${echoedResult}</span>`</span>);
			...
			<p>In the case that you don&#39;t want a single default initialized <code>StitchAppClient</code>,
				you can use the following with as many client app IDs as you&#39;d like to
			initialize clients for multiple app IDs:</p>
			<pre><code class="lang-javascript">Stitch.initializeAppClient(<span class="hljs-string">"&lt;your-client-app-id&gt;"</span>).then(<span class="hljs-function"><span class="hljs-params">client</span> =&gt;</span> {
    <span class="hljs-comment">// use the client</span>
});
</code></pre>
			<p>You can use the client here or anywhere else in your app by using the following:</p>
			...
			<h4 id="closing-the-stitchappclient">Closing the StitchAppClient</h4>
			<p>The client maintains some background processes in the event loop that must be
				shutdown when the client is no longer needed. Simply call <code>close</code> on the client
			when you are done with the client:</p>
			<pre><code class="lang-javascript">client.close();
</code></pre>
		</div>
